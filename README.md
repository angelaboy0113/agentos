...
原因类问题有程序完成门：504、红叉、截图或单条异常记录只能证明现象，必须取得直接业务错误，或至少两类独立证据共同解释作用机制，才允许把原问题标为完成。续轮只把新增环境证据追加给同一 Codex thread，不重发附件和完整历史。环境调查默认每轮 24 次工具调用或 150 秒；数据库重复记录优先使用受控 `group_count`，代码中的求和与两表等值关联可用 `aggregate`、`collect_values`、`linked_aggregate` 在同一轮按参数化只读方式核对，不需要反复抽样。同一环境目标默认最多连续 3 轮，即使每轮都新增少量证据也会停止扩散。开发分析首轮默认 15 分钟、续轮 8 分钟，超时保留已有证据并返回可接续的部分结果，不伪造根因。任务卡显示从源码到环境续轮的累计工具数，不再只显示最后一轮。

若已确认业务状态和安全建议，但请求级日志等外部证据暂不可用，卡片显示橙色部分结果并保留“已确认、未核实、补齐方法”。结果规范化会清除上一轮遗留的退回责任，并在随后恢复前序只读查询记录，因此不会因为核心原因尚未闭环而被通用交接错误覆盖成红卡。MySQL 已开放查询工具时，仅完成连接检查不能结束调查或声称工具不可用。没有任何可靠证据、源码同步失败或结果协议本身无效时仍显示红色受阻。
