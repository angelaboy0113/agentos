...
const initial={outcome:'partial',investigation:{status:'continue',goals:[{id:'history',status:'open',evidence:'Needs live records'},{id:'occupancy',status:'open',evidence:'Needs occupied amounts'}]}};
...
test('URL supplement preserves root goals through environment evidence and rejects false green',async t=>{
 ...
 const resolved={outcome:'ready',investigation:{status:'complete',goals:[{id:'history',status:'verified',evidence:'Actual historical records verified'},{id:'occupancy',status:'verified',evidence:'Actual occupied amounts verified'}]}};
 ...
 assert.equal(again.context.filter(x=>x.result?.investigation?.goals?.[0]?.id==='history').length,1);
});
test('evidence never crosses question, project or chat and direct address questions still complete',async t=>{
 ...
 for(const changes of [{questionId:'other'},{projectId:'other'},{chatId:'other'}]){
  ...
  assert.equal(investigationComplete(j,{investigation:{status:'complete',goals:[{id:'address',status:'verified',evidence:'Configured address verified'}]}}),true);
 }
});
