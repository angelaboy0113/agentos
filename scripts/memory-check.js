...
console.log(JSON.stringify({ mode: 'dry-run', writes: 0, stateSha256: fingerprint(raw.toString()),
  ...
  missionIds: new Set((state.jobs ?? []).map((job) => job.missionId).filter(Boolean)).size, scopes: scopes.size, unknownProjectTurns, eligibleSources: sources }));
