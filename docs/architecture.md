...
支持从问题与环境源码查找入口；Mac 默认接管日常 Chrome 的已登录页面，其他平台或显式配置可使用独立浏览器。等待网站登录时保留任务、登录后自动恢复；网站负责人也可通过私聊凭据入口将账号密码直接写入 Mac Keychain，由固定程序完成登录，模型与任务状态不接触凭据。`BROWSER_BRIDGE` 表示后台进程缺少 macOS Chrome 自动化权限，与网站登录状态分开处理，任务保持等待并在桥接恢复后自动继续。配置、权限、存储与兼容边界见[通用网页排查](query-driven-websites.md)和[日常 Chrome 复用](shared-chrome.md)。
