...
测试使用模拟传输和本地页面，覆盖授权重查、独立标签关联、过期及跨任务引用拒绝、写入及跨域控件过滤、可见文本、密码不读、任务卡归属、负责人校验、机器人 mention 清理、自然格式解析、Keychain 适配、固定登录脚本、登录表单拒绝与二次验证分类。模拟测试不代表真实业务账号接通；每台机器仍需启用系统权限后只读验收。没有向业务群发送测试凭据或点击业务操作。
