...
## 配置与使用
Mac 默认 shared-chrome 后端，使用 Apple Events 接管日常 Google Chrome 的对应应用页面。不复制 Cookie、不读取页面里已有的密码值、不打开远程调试端口、不关闭或重启 Chrome。登录过期时可以本机手动登录，也可以由网站负责人通过私聊凭据入口授权固定程序填写。其他平台默认 isolated；设置 AGENTOS_BROWSER_MODE=isolated 并空闲重启可恢复独立 Playwright 浏览器。
...
当原问题卡显示“等待网页登录”时，当前网站环境在对应机器人 profile 下配置的负责人可以私聊当前项目负责人机器人发送：

```text
网站登录 网址：https://example.com/app/ 账号：你的账号 密码：你的密码
```

当前只有一个网站等待登录时可以省略网址。控制面在普通对话入库和模型处理之前识别该消息，核对发送人、机器人 profile、待登录任务及网站范围，将凭据写入当前 Mac 用户 Keychain，然后用固定登录脚本填写账号、密码并提交。模型不能选择字段、读取凭据或调用这一入口。成功后原问题自动继续；以后会话失效时可从 Keychain 自动尝试一次，不反复提交错误密码。新凭据覆盖该网站原凭据。

凭据只能私聊提交。群聊中的账号密码格式会被 AgentOS 拦截，不进入本地任务和模型，并提醒发送人撤回；AgentOS 无法替用户删除已经发出的飞书消息。原始私聊消息仍受企业飞书消息留存策略管理，AgentOS 不声称删除飞书服务端记录。验证码、短信、扫码、硬件密钥、强制改密和复杂跨域 SSO 不会绕过，页面出现这些步骤时由负责人在本机 Chrome 完成。
...
测试使用模拟传输和本地页面，覆盖授权重查、独立标签关联、过期及跨任务引用拒绝、写入及跨域控件过滤、可见文本、密码不读、私聊负责人校验、Keychain 适配、固定登录脚本和错误分类。模拟测试不代表真实业务账号接通；每台机器仍需启用系统权限后只读验收。没有向业务群发送测试凭据或点击业务操作。
