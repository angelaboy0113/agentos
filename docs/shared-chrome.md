...
也支持 `账号：… 密码：…`、空格、换行、斜杠、逗号等常见写法，不要求记住命令模板。任务卡或同一话题是网站归属依据；没有回复等待登录卡片时，群消息不采用无标签的“两个值”猜测。控制面在普通对话入库和模型处理之前识别该消息，核对发送人、机器人 profile、待登录任务及网站范围，将凭据写入当前 Mac 用户 Keychain，然后用固定登录脚本填写账号、密码并提交。模型不能选择字段、读取凭据或调用这一入口。成功后原问题自动继续；以后会话失效时可从 Keychain 自动尝试一次，不反复提交错误密码。新凭据覆盖该网站原凭据。

群聊回复正文对群成员可见，并受企业飞书消息留存策略管理；AgentOS 不声称删除飞书服务端记录。AgentOS 本机不会把正文写入任务、模型上下文或长期记忆，也不会在回复中复述账号密码。部署团队若不允许在群内出现共享凭据，应继续本机手动登录。验证码、短信、扫码、硬件密钥、强制改密和复杂跨域 SSO 不会绕过，页面出现这些步骤时由负责人在本机 Chrome 完成。
...
测试使用模拟传输和本地页面，覆盖授权重查、独立标签关联、过期及跨任务引用拒绝、写入及跨域控件过滤、可见文本、密码不读、任务卡归属、负责人校验、自然格式解析、Keychain 适配、固定登录脚本和错误分类。模拟测试不代表真实业务账号接通；每台机器仍需启用系统权限后只读验收。没有向业务群发送测试凭据或点击业务操作。
