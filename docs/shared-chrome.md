...
提供脱敏可见正文及筛选后的查看、搜索、分页控件；不读取 Cookie、localStorage、密码输入值及隐藏 noscript 提示。工具使用固定脚本，不接受模型脚本。引用按任务和快照隔离，控件标签或链接变化时失效。写入按钮及跨应用链接不开放。受控只读页面查看自动授权，登录态不会扩大项目、环境或写操作权限。
