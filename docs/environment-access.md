...
工具只从本次 schema 返回的基础表/列中选择。必须有1—6个筛选条件；比较操作符支持 `= > >= < <=`，文本包含用固定 `contains`，有限集合匹配用 `in`（最多100个字符串或数字）。所有值均绑定为参数；禁止空条件全表读取、任意SQL、模型提供函数、联表、写语句或任意目标地址。连接器启用 `supportBigNumbers` 与 `bigNumberStrings`，大于 JavaScript 安全整数范围的业务主键和 DECIMAL 不会先转成浮点数，前一步返回的字符串可直接用于下一步等值或集合筛选。每次最多200行、12列，单值500字符，单工具结果24KB。不限制工具调用总次数；有新证据就继续，连续3次无新增证据时暂停并汇总。
