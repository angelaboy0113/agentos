...
每次只读计划仍绑定目的、环境、命名空间或表范围、参数、行数、超时和配置指纹，不是无限授权。15分钟过期，Runner领取和每次工具调用前重验；过期时按当前配置重新生成并校验只读范围。换环境或扩大范围必须重新校验，但不要求用户重复回复“同意”。
...
工具只从本次 schema 返回的基础表/列中选择。必须有1—6个比较条件，操作符仅 `= > >= < <=`，值绑定为参数；禁止空条件全表读取、任意SQL、函数、联表、写语句或任意目标地址。连接器启用 `supportBigNumbers` 与 `bigNumberStrings`，大于 JavaScript 安全整数范围的业务主键和 DECIMAL 不会先转成浮点数，前一步返回的字符串可直接用于下一步等值筛选。每次最多200行、12列，单值500字符，单工具结果24KB。不限制工具调用总次数；有新证据就继续，连续3次无新增证据时暂停并汇总。
