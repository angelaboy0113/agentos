...
不再采用旧版整个任务 8 次或 12 次的硬截断；限制作用于一次环境调查轮，后续只有在源码分析提出新的精确目标时才会继续。模型每次仅接收最近 12 项工具结果，完整证据仍由本地状态保存。无进展或预算用尽会给出已有结论及下一步缺口。此项不等于已接通代码、日志与数据库的自动联合排查，跨资源访问仍由现有任务路由和授权决定。
