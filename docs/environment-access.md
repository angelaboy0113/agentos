...
工具只从本次 schema 返回的基础表/列中选择。必须有1—6个比较条件，操作符仅 `= > >= < <=`，值绑定为参数；禁止空条件全表读取、任意SQL、函数、联表、写语句或任意目标地址。每次最多200行、12列，单值500字符，单工具结果24KB。不限制工具调用总次数；有新证据就继续，连续3次无新增证据时暂停并汇总。
...
investigate不再使用固定调用次数：新建模板不写maxCalls，旧模板字段兼容保留但不再执行次数上限。无需手工改本地配置；升级程序并空闲重启即可。单次超时、行数、权限、授权期限、Runner停止与租约仍生效，不承诺无限时长。
...
不再限制整个任务只调用8次或12次工具。回归验证16次有新证据的调用可完成，旧maxCalls=1也不截断；数据库与浏览器工具内部同步取消总次数限制。无进展暂停仍会给出已有结论及下一步缺口。此项不等于已接通代码、日志与数据库的自动联合排查，跨资源访问仍由现有任务路由和授权决定。
