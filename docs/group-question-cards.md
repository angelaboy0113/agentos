...
- 多人可以各自提问，受理按问题/机器人/角色/项目分别调度；耗时 Job 与受理线程分离。Runner 池默认提供 3 个真实执行槽，不同卡片可以同时运行；超过槽数的任务按队列等待。可用 `AGENTOS_RUNNER_CONCURRENCY=1..8` 调整，实际值在 `/health` 和本机管理控制台显示。
- 主卡的“总耗时”从用户发送原问题开始累计。后续源码调查、网页核验或数据库只读查询即使进入新的执行步骤，也不重新从零计时。环境排查每 10 秒发送一次进度心跳以刷新执行中的卡片；飞书传输可能产生数秒延迟，因此不承诺逐秒跳动。任务结束后保留最终累计时间。
...
### Evidence-driven investigation across stages

New jobs preserve the user's `originalQuestion` separately from the model's first-stage instruction. Source investigators and owner reviewers both receive the configured environment catalog and may request a matching environment query. The next job remains on the same question, with original sender, project, attachments and prior evidence. A new environment request always waits for its scope approval; source permissions never imply production access.

For partial environment results, if the project has `analysisRepositories`, the controller atomically queues another read-only developer investigation with the runtime evidence and the original goal. It removes the prior environment grant and chooses the source environment from the queried tier. No source configuration means no automatic source continuation. Completed/blocked queries do not use this edge. Duplicate/late completion events are rejected by leases, and identical environment/template/parameters already requested on that question are not automatically replayed. Three runtime rounds with identical result hashes stop continuation. These guards do not guarantee semantic progress when different queries return different but irrelevant evidence.

MySQL investigate templates expose `count` alongside schema and select. It requires known base-table fields and explicit validated filters, supports row count or distinct field combinations, and returns a count without the select sample-row limit. Distinct combinations containing NULL follow MySQL COUNT DISTINCT semantics and are excluded. It does not accept raw SQL, arbitrary functions or joins, and retains the approved per-operation deadline and read-only transaction. Source/owner prompts require actual counts rather than extrapolating samples.

This is not full desktop-tool parity. Available live tools remain scoped MySQL and Nacos (including its browser adapter). Generic business websites, arbitrary application logs, arbitrary SQL/joins, and mutation operations are not automatically available. When evidence requires an unsupported capability, the agent must identify the missing system/tool and required cooperation instead of pretending it inspected it. Model selection of the next request is guided by prompts; execution scope, approval and task isolation are enforced in code. No guarantee of identical answers to interactive Codex is made.

Validation: automated source-owner escalation, runtime-to-source continuation, grant stripping, preserved original question and evidence, duplicate completion and repeated-scope rejection, validated aggregate SQL and actual adapter count results. Existing historical jobs are not automatically restarted.
