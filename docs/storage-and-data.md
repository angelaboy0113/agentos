...
状态文件使用紧凑 JSON 和有界问题上下文。每个在途 Job 最多携带 8 项去重证据；已经交接的历史 Job 保留自身结果与事件，不再重复保存整条上下文。主卡只装载最近 6 项有效结果，避免长任务把卡片和每次状态事务拖慢。升级迁移只删除重复的上下文副本，不删除 Job 结果、事件或本机审计历史；迁移前仍应按下文要求停机备份。
