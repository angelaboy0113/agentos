...
export async function executeEnvironmentJob(job, config, emit) {
  ...
  let result;
  try { result = await readEnvironment(plan); }
  catch (error) { return { outcome: 'blocked', summary: error.message, finalMessage: error.message, verification: [] }; }
  const safeSummary = `${plan.tier.toUpperCase()} · ${plan.description}：已查询 ${result.rows.length} 条${result.truncated ? '（结果受限，非全部数据）' : ''}。读取时间：${result.evidence.readAt}。${result.note ?? ''}`;
  let explanation = '';
  ...
  return { outcome: 'ready', summary: safeSummary, finalMessage: [safeSummary, explanation, rows].filter(Boolean).join('\n\n'),
