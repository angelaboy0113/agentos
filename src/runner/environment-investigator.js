...
const boundedInt=(value,fallback,min,max)=>{const n=Number(value);return Number.isInteger(n)&&n>=min&&n<=max?n:fallback;};
export const environmentInvestigationLimits=()=>({
  maxCalls:boundedInt(process.env.AGENTOS_ENV_INVESTIGATION_MAX_CALLS,24,4,60),
  maxMs:boundedInt(process.env.AGENTOS_ENV_INVESTIGATION_MAX_MS,150000,30000,600000),
});
...
export async function investigateEnvironment(plan, emit = async () => {}, adapters = {}) {
  ...
  const limits=adapters.limits??environmentInvestigationLimits(),began=Date.now();
  ...
  try {
    ...
    try {
    for (let i = 0; ; i++) {
      if(results.length>=limits.maxCalls||Date.now()-began>=limits.maxMs){stopReason=`环境调查已达到本轮性能预算（最多${limits.maxCalls}次工具或${Math.round(limits.maxMs/1000)}秒），已保留现有证据并停止扩散查询。`;break;}
