...
import { createEnvironmentTools } from './environment-tools.js';
...
export async function investigateEnvironment(plan, emit = async () => {}, adapters = {}) {
  ...
  let planner; const steps = [], results = []; let summary = '', complete = false;
  try {
    ...
    for (let i = 0; i < q.maxCalls; i++) {
      ...
      const result = await tools.run(choice.tool, args);
    ...
    if (results.length < 2 || results.some(x => x.result.partial)) complete = false;
